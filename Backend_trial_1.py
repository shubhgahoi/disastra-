...
# UPDATED: The "Brain" for our bot is now smarter
# It's a list of tuples. The first item in the tuple is a list of keywords to check for.
# The bot will only respond if ALL keywords in the list are present in the user's message.
# The list is checked from top to bottom, so more specific rules should be at the top.
BOT_LOGIC = [
    ...
    (["live", "alerts"], "The Live Alerts section shows recent natural events around the world (or near you if location is enabled) using data from NASA."),
    (["emergency", "kit"], "Your emergency kit should contain water, food, a first-aid kit, a flashlight, and batteries. Use the 'Emergency Kit' checklist on the dashboard to track your items."),
    ...
    # Default response is now handled in the function itself
]


# --- HELPER FUNCTIONS & OTHER ROUTES (No changes below this line) ---
...
# UPDATED: Chatbot route with smarter logic
@app.route('/chatbot', methods=['POST'])
def handle_chatbot():
    user_message = request.json.get('message', '').lower()
    
    # Loop through our logic rules
    for keywords, response in BOT_LOGIC:
        # Check if all keywords for a rule are in the user's message
